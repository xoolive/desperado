...
async fn main() -> tokio::io::Result<()> {
    ...
    while let Some(chunk) = iq_file.next().await {
        ...
        /*while start_idx < deemphasized.len() {
            let end_idx = (start_idx + max_input_frames).min(deemphasized.len());
            let chunk_to_resample = &deemphasized[start_idx..end_idx];

            let input_block = vec![chunk_to_resample.to_vec()];

            // Adjust resample ratio LESS frequently and LESS aggressively
            adjustment_counter += 1;
            if adjustment_counter >= ADJUSTMENT_INTERVAL {
                adjustment_counter = 0;

                let fill_ratio = tx.len() as f64 / (AUDIO_RATE * 2) as f64;
                let target_fill = 0.5; // Target 50% buffer fill
                let error = fill_ratio - target_fill;

                // Only adjust if error is significant (>10% from target)
                /*if error.abs() > 0.1*/
                {
                    // Much gentler adjustment: 0.01% per adjustment
                    let adjustment = 1.0 + (error * 0.0002);

                    // If buffer is filling up (error > 0), we need to OUTPUT MORE audio
                    // → INCREASE resample ratio (more output samples per input)
                    // If buffer is emptying (error < 0), we need to OUTPUT LESS audio
                    // → DECREASE resample ratio
                    resample_ratio *= adjustment;

                    // Clamp to reasonable bounds (±2% from nominal)
                    let nominal = AUDIO_RATE as f64 / FM_BANDWIDTH as f64;
                    resample_ratio = resample_ratio.clamp(nominal * 0.98, nominal * 1.02);

                    resampler.set_resample_ratio(resample_ratio, true).unwrap();
                    print!(
                        "\rBuffer fill: {:.1}%, Resample ratio: {:.6} (error: {:.3})      ",
                        fill_ratio * 100.0,
                        resample_ratio,
                        error
                    );
            ...
            // Resample chunk
            let output_block = resampler.process(&input_block, None).unwrap();
            let audio_samples = &output_block[0];

            // 7. Send to audio output with volume normalization
            /*let max_val = audio_samples.iter().fold(0.0_f32, |a, &b| a.max(b.abs()));
            let scale = if max_val > 0.0 { 0.8 / max_val } else { 1.0 };

            for &s in audio_samples {
                let scaled = s * scale;
                if tx.try_send(scaled).is_err() {
                    println!("Audio buffer full, dropping samples");
                    break;
                }
            }*/

            // 7. Improved AGC with smoother gain control
            let chunk_max = audio_samples.iter().fold(0.0_f32, |a, &b| a.max(b.abs()));

            // Update AGC gain
            if chunk_max > 0.0001 {
                let target_gain = 0.5 / chunk_max; // Target 0.5 instead of 0.8 to prevent clipping
                if target_gain < agc_gain {
                    // Attack: reduce gain quickly if signal is too loud
                    agc_gain = agc_gain * AGC_ATTACK + target_gain * (1.0 - AGC_ATTACK);
                } else {
                    // Release: increase gain slowly if signal is quiet
                    agc_gain = agc_gain * AGC_RELEASE + target_gain * (1.0 - AGC_RELEASE);
                }
            ...
            for &s in audio_samples {
                ...
                if tx.try_send(clipped).is_err() {
                    // Buffer full - this shouldn't happen often now
